...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			throw new TreeException("There is no parent name."); //throw a TreeException with a useful message
		...
		else
		{
			...
			if(parentNode == null) {
				throw new TreeException("Parent wasn't found.");		// the parent node wasn't found, there must have been something wrong in the 
			}
		...
		for(String child: childrenArray) {
			TreeNode currToAdd = new TreeNode(child);
			parentNode.addChild(currToAdd);
		}
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("First node does not exist within the tree."); //Throw a TreeException with a useful message
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("Second node does not exist within the tree."); //Throw TreeException with a useful message
